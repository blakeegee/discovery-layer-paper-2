...
searchit_titles=read.csv("D:/Rstudio projects/tidy_string_distance/Fall_2020/No_ref_no_news/paperTitles_inSearchItYes_Fall2020_noRefnoNewsnoReview_oneObsPerRow_papernumbers.csv")
...
source_titles=read.csv("D:/Rstudio projects/tidy_string_distance/Fall_2020/fall_2020_paper_citations_05.csv")
...
colnames(jaccard) <- c("Paper Number","biblography title","Bibliography Type","Query Number","Search It Title","Search It Type","Jaccard Score")
while (n  < 50  ) {
#n should be the number of source citations to check (citations from papers, for fall 2020 citation count is 442 -- but for file size the one file has been broken up into 5 files -- 4 at 100 citations a piece and the last with 50)
#b should be the whole number of titles returned from queries to check  (titles returned from discovery layer to compare against. for fall 2020 this will be different based on the facets and the queries.....)
while (b < 4302) {
  new_row <-  c(matrix_source_title[i,1],matrix_source_title[i,4],matrix_source_title[i,6],matrixsearchit_titles[b,1],matrixsearchit_titles[b,4],matrixsearchit_titles[b,5],stringdist(tolower(matrix_source_title[i,4]),tolower(matrixsearchit_titles[b,4]), method = "jaccard", q = "7"))
  b = b + 1
...
```
```{r}
write.csv(jaccard,"D:\\Rstudio projects\\tidy_string_distance\\jaccard_q7_no_news_no_eviews_fall_05.csv")
