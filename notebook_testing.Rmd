...
source_titles=read.csv("D:/Rstudio projects/tidy_string_distance/fall_2020_paper_citations_seeded_with_wins.csv")
...
i <- 1
b <- 1
jaccard <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(jaccard) <- c("Source Title","Source Type","Search It Title","Search It Type","Jaccard Score")
while (n < 5   ) {
...
while (b < 4275) {
  new_row <-  c(matrix_source_title[i,4],matrix_source_title[i,6],matrixsearchit_titles[b,3],matrixsearchit_titles[b,4],stringdist(tolower(matrix_source_title[i,4]),tolower(matrixsearchit_titles[b,3]), method = "jaccard", q = "7"))
  b = b + 1
...
```
```{r}
write.csv(jaccard,"D:\\Rstudio projects\\tidy_string_distance\\jaccard_7.csv")
