...
```{r}
searchit_titles=read.csv("D:/Rstudio projects/tidy_string_distance/one_paper_query_results_test.csv")
matrixsearchit_titles=as.matrix(searchit_titles)
vectorsearchit_titles=as.vector(searchit_titles)

papers_info_test=read.csv("D:/Rstudio projects/tidy_string_distance/papers_info_test.csv")
matrixpapers_info_test=as.matrix(papers_info_test)
vectorpapers_info_test=as.vector(papers_info_test)
...
colnames(jaccard) <- c("Citation Title","Search It Title","Jaccard Score")
while (n < 11) {

while (b < 51) {
  new_row <-  c(matrixpapers_info_test[i,4],matrixsearchit_titles[b,3],stringdist(tolower(matrixpapers_info_test[i,4]),tolower(matrixsearchit_titles[b,3]), method = "jaccard"))
  b = b + 1
